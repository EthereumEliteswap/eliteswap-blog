...
const APOLLO_QUERY = gql`
  {
    eliteswapFactory(id: "0x81c27D1A0a6413c6A6d6d8e54B0A9C869d16771C") {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
    }
...
export const ELITESWAP_GLOBALS_24HOURS_AGO_QUERY = block => {
  let queryString = `
  query eliteswapFactory {
    eliteswapFactory(id: "0x81c27D1A0a6413c6A6d6d8e54B0A9C869d16771C", block: { number: ${block} }) {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
...
export default function Ticker() {
  ...
  useEffect(() => {
    async function getData() {
      let result = await client.query({
        query: ELITESWAP_GLOBALS_24HOURS_AGO_QUERY(oneDayBackBlock),

        fetchPolicy: 'cache-first'
      })
      if (result) {
        setOnedayResult(result?.data?.eliteswapFactory)
      }
    }
    if (oneDayBackBlock) {
      getData()
  ...
  if (data && oneDayResult) {
    const volume24Hour = parseFloat(data?.eliteswapFactory?.totalVolumeUSD) - parseFloat(oneDayResult?.totalVolumeUSD)
    ...
    EltStats.liquidity = [
      ...
      }).format(data.eliteswapFactory.totalLiquidityUSD),
      ' Total Liquidity'
    ]
    EltStats.exchanges = [Number.parseFloat(data?.eliteswapFactory?.pairCount), ' Total Pools']
  ...
  useLayoutEffect(() => {
    if (loading === false && EltStats.volume !== undefined) {
      updateInitialized(true)
    }
  }, [loading, EltStats.volume])
      ...
      <MarqueeWrapper href="https://uniswap.info/" className="ticker" data-speed="0.25" data-pausable="true">
        <div>
          {Array.from({ length: totalElements }).map((_, idx) => {
            return <AnimatingEl stat={EltStats.key((idx % 4) + 1)} key={idx} />
          })}
        </div>
      </MarqueeWrapper>
    )
